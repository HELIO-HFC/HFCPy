...
class Viewer(tk.Frame):
    ...
    def __init__(self, master=None, **kwargs):
        ...
        self._obsid.set(self.__obs2id(self._obs, self._inst,
                                      telescope=self._tele, wavename=self._wave))
        self._wsdl = kwargs.pop("url_wsdl", URL_WSDL)
        self._quiet = kwargs.pop("Quiet", False)
        self._dev = kwargs.pop("Dev", False)
        if (self._dev):
            self._wsdl = URL_WSDL_DEV
        ...
        self._aron = tk.IntVar()  # Active regions
        self._chon = tk.IntVar()  # Coronal holes
        self._spon = tk.IntVar()  # Sunspots
        self._rson = tk.IntVar()  # NRH radio sources
        self._pron = tk.IntVar()  # Prominences
        self._fion = tk.IntVar()  # Filaments
    ...
    def __place_widgets(self):
        ...
        # Date buttons
        lbtn = tk.Button(hframe, text="Previous",
                      bg=BG_COLOR,
                      highlightbackground=BG_COLOR,
                      command=self.__prev_date)
        rbtn = tk.Button(hframe, text="Next",
                      bg=BG_COLOR,
                      highlightbackground=BG_COLOR,
                      command=self.__next_date)
        # Date entry
        self._header = tk.Entry(hframe,
                             highlightbackground=BG_COLOR,
        ...
        for text, mode in MODES:
            b = tk.Radiobutton(dframe, text=text,
        ...
        cb_ar = tk.Checkbutton(fframe, text="Active regions",
                            variable=self._aron, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_ch = tk.Checkbutton(fframe, text="Coronal holes",
                            variable=self._chon, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_sp = tk.Checkbutton(fframe, text="Sunspots",
                            variable=self._spon, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_rs = tk.Checkbutton(fframe, text="NRH sources",
                            variable=self._rson, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_pr = tk.Checkbutton(fframe, text="Prominences",
                            variable=self._pron, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_fi = tk.Checkbutton(fframe, text="Filaments",
                            variable=self._fion, bg=BG_COLOR,
                            command=self.__plot_qclk)
        cb_ar.pack(anchor=tk.W)
        cb_ch.pack(anchor=tk.W)
        cb_sp.pack(anchor=tk.W)
        cb_rs.pack(anchor=tk.W)
        cb_pr.pack(anchor=tk.W)
        cb_fi.pack(anchor=tk.W)
    ...
    def __id2obs(self, id):
        ...
        if (id == 1):
            ...
            messagebox.showerror(
                "ERROR", "UNKNOWN OBSERVATORY ID %i!" % (id))
    ...
    def __set_date(self):
        ...
        logger.info("Loading information for the data set:")
        logger.info("(DATE_OBS=%s, OBSERVAT=%s, INSTRUME=%s, TELESCOP=%s, WAVENAME=%s)" % (
            date_obs, obs, inst, tel, wav))
    ...
    def __prev_date(self):
        ...
        LIMIT = 1
        ORDER = "DATE_OBS DESC"
        WHERE = "(OBSERVAT=\"%s\") AND (INSTRUME=\"%s\") AND (DATE_OBS < \"%s\")" % (
            obs, inst, sql_date(date_obs))
        if (tel != ""):
            WHERE += " AND (TELESCOP=\"%s\")" % (tel)
        if (wav != ""):
            WHERE += " AND (WAVENAME=\"%s\")" % (wav)
    ...
    def __load_qclk(self):
        ...
        logger.info("Loading quicklook image from %s" % (self.qclk_url))
    ...
    def __plot_qclk(self):
        ...
        cdelt1 = np.float(data['CDELT1'])
        cdelt2 = np.float(data['CDELT2'])
        crpix1 = np.float(data['CENTER_X'])
        crpix2 = np.float(data['CENTER_Y'])
        wavemin = str(data['WAVEMIN'])
        waveunit = str(data['WAVEUNIT'])
        ...
        if (self._aron.get()):
            logger.info("Plotting active region data")
            self.__plot_feat("ar")
        if (self._chon.get()):
            logger.info("Plotting coronal hole data")
            self.__plot_feat("ch")
        if (self._spon.get()):
            logger.info("Plotting sunspot data")
            self.__plot_feat("sp")
        if (self._rson.get()):
            logger.info("Plotting radio source data")
            self.__plot_feat("rs")
        if (self._pron.get()):
            logger.info("Plotting prominence data")
            self.__plot_feat("pr")
        if (self._fion.get()):
            logger.info("Plotting filament data")
            self.__plot_feat("fi")
    ...
    def __plot_feat(self, feature):
        ...
        for current_feat in feat_data:
            ...
            if (track_id > 0) and (self._feat_ct.get() == 1):
                ...
                feat_color = [float(r) / 256.0, float(g) /
                              256.0, float(b) / 256.0]
    ...
    def __load_feat(self, feature):
        ...
        WHERE = "(DATE_OBS BETWEEN \"%s\" AND \"%s\")" % (
            sql_date(starttime), sql_date(endtime))
        FROM = feat_table
        ...
        for td in feat_data.tabledata:
            ...
            if (dt == dt_min):
                tabledata.append(td)
        logger.info("HFC %s data found for the date %s" %
              (feat.upper(), tabledata[0]['DATE_OBS']))
